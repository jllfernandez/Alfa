...
@Controller
@Slf4j
@RequestMapping(path = Constants.AUTO_MAPPING)
public class BeiController extends CommonController {
	...
	public String entrada(Model model) {
		...
		List<Role> lista = rolesService.findAll();
		System.out.println("  --->" + lista.size());
		List<Capability> caps = lista.get(0).getCapabilities();
...
//		rtn.put("result", new Gson().toJson(treeSvc.getTree()));
	...
	public ResponseEntity<?> findAll(@Valid @RequestBody UsuarioFilter filter) throws Exception {
		...
		ArrayList<Usuario> entitys = (ArrayList<Usuario>) filtrado.get("content");
		// Paged paginacion = (Paged) filtrado.get("paginacion");
		List<UsuarioDTO> lista = mapper.usuariosFlat(entitys);
	...
	public ResponseEntity<?> create(@Valid @RequestBody UsuarioDTO usuario) throws Exception {
		logger.debug("Data received at create: {}", usuario);

		Usuario usuarioBack = usuariosService.create(mapper.dtoToUsuario(usuario));
		return new ResponseEntity<>(mapper.usuarioToDto(usuarioBack), HttpStatus.CREATED);
